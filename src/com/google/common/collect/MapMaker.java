 ...
 * <p> {@code new MapMaker().weakKeys().makeMap()} can almost always be used as
 * a
...
public final class MapMaker
{
    ...
    {
        return useCustomMap ? new StrategyImpl<K, V>(this).map
            : new ConcurrentHashMap<K, V>(builder.getInitialCapacity(), 0.75f,
     ...
     * completes.
     * <p> {@link Map#get} on the returned map will never return {@code null}.
     * It
     * may throw:
     * <ul>
     ...
     * argument is of type {@code K}. The {@code get} method accepts
     * {@code Object}, so the key type is not checked at compile time. Passing
     * an
    ...
    private enum Strength
    {
        WEAK
        {
            ...
            {
                return (next == null) ? new WeakEntry<K, V>(internals, key,
                    hash) : new LinkedWeakEntry<K, V>(internals, key, hash,
                    next);
            }
            
            @Override
            <K, V> ReferenceEntry<K, V> copyEntry(K key,
                ReferenceEntry<K, V> original, ReferenceEntry<K, V> newNext)
            {
                WeakEntry<K, V> from = (WeakEntry<K, V>) original;
                return (newNext == null) ? new WeakEntry<K, V>(from.internals,
                    key, from.hash) : new LinkedWeakEntry<K, V>(from.internals,
                    key, from.hash, newNext);
            }
        },
        ...
        SOFT
        {
            ...
            <K, V> ReferenceEntry<K, V> newEntry(
                ...
                int hash, ReferenceEntry<K, V> next)
            {
                return (next == null) ? new SoftEntry<K, V>(internals, key,
                    hash) : new LinkedSoftEntry<K, V>(internals, key, hash,
                    next);
            }
                ...
                ReferenceEntry<K, V> original, ReferenceEntry<K, V> newNext)
            {
                ...
                return (newNext == null) ? new SoftEntry<K, V>(from.internals,
                    key, from.hash) : new LinkedSoftEntry<K, V>(from.internals,
                    key, from.hash, newNext);
            }
        },
        ...
        STRONG
        {
            ...
            <K, V> ReferenceEntry<K, V> newEntry(
                ...
                int hash, ReferenceEntry<K, V> next)
            {
                return (next == null) ? new StrongEntry<K, V>(internals, key,
                    hash) : new LinkedStrongEntry<K, V>(internals, key, hash,
                    next);
            }
                ...
                ReferenceEntry<K, V> original, ReferenceEntry<K, V> newNext)
            {
                ...
                return (newNext == null) ? new StrongEntry<K, V>(
                    from.internals, key, from.hash)
        ...
        ComputingStrategy<K, V, ReferenceEntry<K, V>>
    {
        ...
        class WeakRefTask extends TimerTask
        {
            K key;
            V value;
            WeakRefTask(K key, V value){
                this.key = key;
                this.value = value;
            }
            /**
            ...
            public void run()
            {
                //Constants.print("MapMaker", "["+key.toString() + "-->gc]");
                WeakReference<K> keyReference = new WeakReference<K>(key);
                WeakReference<V> valueReference = new WeakReference<V>(value);
        ...
        void scheduleRemoval(K key, V value)
        {
            ...
            instance.schedule(new WeakRefTask(key,value),
            ...
            ReferenceEntry<K, V> original, ReferenceEntry<K, V> newNext)
        {
            ...
            if (valueReference == COMPUTING)
            {
                ...
                newEntry.setValueReference(new FutureValueReference(original,
                    newEntry));
                return newEntry;
            }
            else
            ...
            Function<? super K, ? extends V> computer)
        {
            ...
            {
                setValueReference(entry,
                    new ComputationExceptionReference<K, V>(t));
        ...
        private class FutureValueReference implements ValueReference<K, V>
        {
            ...
            FutureValueReference(ReferenceEntry<K, V> original,
                ReferenceEntry<K, V> newEntry)
            {
    ...
    private static class NullOutputExceptionReference<K, V> implements
        ValueReference<K, V>
    {
    ...
    private static class ComputationExceptionReference<K, V> implements
        ValueReference<K, V>
    {
        final Throwable t;
    ...
    {
        LinkedSoftEntry(Internals<K, V, ReferenceEntry<K, V>> internals, K key,
            int hash, ReferenceEntry<K, V> next)
        {
            super(internals, key, hash);
            this.next = next;
    ...
    {
        LinkedWeakEntry(Internals<K, V, ReferenceEntry<K, V>> internals, K key,
            int hash, ReferenceEntry<K, V> next)
        {
            super(internals, key, hash);
            this.next = next;
